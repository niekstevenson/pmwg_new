...
group_dist_factor = function(random_effect = NULL, parameters, sample = FALSE, n_samples = NULL, info){
  ...
  param.lambda <- unwind_lambda(param.lambda.unwound, info$hyper$constraintMat, info$n_factors, reverse = T)
  ...
  if (sample){
    ...
    return(mvtnorm::dmvnorm(random_effect, param.theta_mu,param.var,log=TRUE))
  }
}
...
prior_dist_factor = function(parameters, info){ 
  ...
  log_prior_sig_err_inv = sum(dgamma(param.sig_err_inv, shape = hyper$nu/2, rate = (hyper$s2*hyper$nu)/2, log=TRUE))
  log_prior_psi_inv = sum(dgamma(param.psi_inv, shape = hyper$al, rate = hyper$bl, log=TRUE))
