...
get_conditionals_inft_factor <- function(s, samples, n_pars){
  iteration <- samples$iteration
  sig_err <- log(samples$theta_sig_err_inv)
  psi <- log(samples$theta_psi)
  eta <- matrix(samples$theta_eta[s,,], nrow = samples$n_factors)
  lambda <- apply(samples$lambda_untransf, 3, unwind_lambda, samples$n_factors)
  theta_mu <- samples$theta_mu 
  all_samples <- rbind(samples$alpha[, s,],theta_mu, eta, sig_err, psi, lambda)
...
variant_funs <- list(
  ...
  get_conditionals = get_conditionals_inft_factor,
