...
extend_sampler <- function(sampler, n_samples, stage) {
  ...
  new_beta = array(NA_real_,dim = dim(old$theta_beta) + c(0, 0, n_samples), dimnames = list(colnames(covariates), NULL, NULL))
  new_beta[,,-(start:end)] <- old$theta_beta
  sampler$samples$theta_beta <- new_beta 
...
trim_na <- function(sampler) {
  ...
  sampler$samples$theta_beta <- sampler$samples$theta_beta[, , 1:idx, drop = F]
  sampler$samples$theta_eta <- sampler$samples$theta_eta[, , 1:idx, drop = F]
...
sample_store <- function(par_names, subject_ids, covariates, n_factors, iters = 1, stage = "init") {
...
last_sample <- function(store) {
  list(
    ...
    beta = store$theta_beta[,,store$idx]
  )
}
