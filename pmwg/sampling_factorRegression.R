...
source("pmwg/utils_factorRegression.R")
source("pmwg/messaging.R")

pmwgs <- function(data, covariates, pars, ll_func, n_factors, prior = NULL, constraintMat = NULL) {
  ...
  samples <- sample_store(pars, subjects, covariates, n_factors)
  # Checking and default priors
  if (is.null(prior)) {
    prior <- list(theta_mu_mean = rep(0, n_pars), 
                  theta_mu_var = rep(1, n_pars),
                  theta_lambda_var = 1,
                  theta_beta_mean = matrix(0, ncol(covariates), n_factors),
                  theta_beta_sigma = matrix(1, ncol(covariates), n_factors)
    )
  }
...
init <- function(pmwgs, start_mu = NULL, start_var = NULL, start_psi_inv = NULL, 
                 start_sig_err_inv = NULL, start_lambda = NULL, start_beta = NULL,
  ...
  pmwgs$samples$theta_beta[,,1] <- start_beta
...
gibbs_step_factor_regr <- function(sampler){
  ...
  return(list(tmu = mu + t(lambda%*%beta[1,]), tvar = var, lambda = lambda, lambda_orig = lambda_orig, eta = eta,
              sig_err_inv = sig_err_inv, psi_inv = psi_inv, beta = beta, alpha = last$alpha))
}
...
run_stage <- function(pmwgs,
  ...
  for (i in 1:iter) {
    ...
    pars <- gibbs_step_factor_regr(pmwgs)
    if(n_cores > 1){
    ...
    pmwgs$samples$theta_beta[, , j] <- pars$beta
